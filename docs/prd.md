...
随着无服务器架构的兴起，传统的中心化协作平台已无法满足分布式团队的需求。DIPCP旨在构建一个基于GitHub的无服务器协作平台，为开源项目提供内容管理和协作功能。

### 1.2 产品定位

DIPCP是一个**无服务器协作平台**，包括桌面版、手机版和网页版。系统采用零运维成本架构，所有业务逻辑在客户端实现，通过GitHub仓库存储所有数据。产品专注于内容创作、版本管理、审核流程和积分奖励机制。
...
#### 2.7.1 用户注册与认证
- **登录方式**: 用户输入用户名、仓库地址和GitHub Personal Access Token
